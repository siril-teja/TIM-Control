...
function results = for_cost_function_3()
...
optimization_data.U = []; 
   ...
   % Define 6 colors for different initial conditions
    ...
    hold on;
    xlabel('U = max(|X|, |Y|)');
    ylabel('Sum of Square of IVD Forces (x 10E-5)');
...
for i = 1:length(initial_values)
    ...
    Requirements = struct;
    bnds = getbounds('ROM_draft3/Minimize SumofSquareofIVDForces');
    ...
    Sig_Info.BlockPath = 'ROM_draft3/Minimize SumofSquareofIVDForces/u';
    Sig_Info.LoggingInfo.LoggingName = 'Sig-Cost3';
...
scatterPlots(dataset_index).YData = [scatterPlots(dataset_index).YData, Cost3];
