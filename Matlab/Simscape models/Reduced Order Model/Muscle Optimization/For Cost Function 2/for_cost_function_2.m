...
function results = for_cost_function_2()
...
optimization_data.U = []; 
   ...
   % Define 6 colors for different initial conditions
    ...
    hold on;
    xlabel('U = max(|X|, |Y|)');
    ylabel('Sum of Muscle Stress (MPa)');
...
for i = 1:length(initial_values)
    ...
    Requirements = struct;
    bnds = getbounds('ROM_draft3/Muscle Forces/Minimize SumofMuscleStress');
    ...
    Sig_Info.BlockPath = 'ROM_draft3/Muscle Forces/Minimize SumofMuscleStress/u';
    Sig_Info.LoggingInfo.LoggingName = 'Sig-Cost2';
...
scatterPlots(dataset_index).YData = [scatterPlots(dataset_index).YData, Cost2];
