...
spk = 1000;%spring constants

act_top=(n/2);%node at which top tensions act
act_bottom= (n/2)+1;%node at which bottom tensions act
...
orient= sym('orient',[n-1 6]);
%Orientation matrix
orient_sol = sym('orient_sol',[1 (n-1)*12]);

for i=1:n-1
    for j=1:6
        orient(i,j)= orient_sol(6*(i-1)+j);
    end
end
...
eqn_1=force_eqn/m;
eqn_2=moment_eqn/I;
eqn_all=sym(zeros(1,6*(n-1)));
for i=1:n-1
    for j=1:3
        eqn_all(1,6*(i-1)+j)= force_eqn(i,j);
        eqn_all(1,6*(i-1)+j+3)= moment_eqn(i,j);
    end
end


Gov_eqn= matlabFunction(eqn_all);

state_ini=zeros(1,12*(n-1));
t_span=0:0.02:50;
[t,X_sol]= ode45(@(t,orient_sol)fun(t,orient_sol,Gov_eqn,n),t_span,state_ini);

function orient_sol_dot = fun(t,orient_sol,Gov_eqn,n)
temp= Gov_eqn(orient_sol(1),orient_sol(2),orient_sol(3),orient_sol(4),orient_sol(5),orient_sol(6),orient_sol(7),orient_sol(8),orient_sol(9),orient_sol(10),orient_sol(11),orient_sol(12),orient_sol(13),orient_sol(14),orient_sol(15),orient_sol(16),orient_sol(17),orient_sol(18))
for i=1:n-1
    for j=1:6
        orient_sol_dot(6*(i-1)+j,1)=orient_sol(6*(n-1)+6*(i-1)+j,1);
        orient_sol_dot(6*(i-1)+j+6*(n-1),1)= temp(6*(i-1)+j);
    end
end
end
